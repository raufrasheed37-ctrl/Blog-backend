  ...
  const posts = [
    {
      ...
      handle: "@arimiles",
      time: "2h",
      text: "The best ideas usually come from the notes you almost threw away. I turned one rough sketch into a paid weekly newsletter and crossed my first 500 subscribers this month.",
    ...
    },
    {
      ...
      handle: "@noahwrites",
      time: "5h",
      text: "Creators who publish consistently for 90 days have the highest retention lift. Sharing my exact framework for drafting, editing, and shipping in under two hours.",
        ...
        <main className="w-full min-w-0 flex-1 space-y-5 rounded-3xl border border-white/10 bg-[#0f0f0f] p-4 shadow-[0_30px_70px_-40px_rgba(0,0,0,0.95)] sm:p-5 lg:p-6">
          ...
          <section className="space-y-4">
            {posts.map((post) => (
              <article
                key={post.handle}
                className="rounded-3xl border border-white/10 bg-[#161616] p-5 shadow-[0_22px_45px_-34px_rgba(0,0,0,0.95)]"
              >
                <div className="flex items-center gap-3">
                  ...
                  />
                  <div>
                    <p className="text-sm font-semibold text-zinc-100">{post.name}</p>
                    <p className="text-xs text-zinc-500">
                      {post.handle} - {post.time}
                    </p>
                  </div>
                </div>
                <p className="mt-4 text-sm leading-7 text-zinc-300">{post.text}</p>
              </article>
            ))}
          </section>
